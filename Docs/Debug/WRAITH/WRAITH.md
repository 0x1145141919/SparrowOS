...
> **状态**：`疑似已解决 / 【未结案】（2026-09-17）`——根因 = **双调度/多调度**（阻塞/退出者「已发布、未切离」期间被异核唤醒+偷取 → 同栈并发）。修复 F1–F5（`67e4a76`）+ 同配方 A/B（pre-fix `pre08` 命中 / post-fix **0/40**）；**结案条件（调度器单元压测 / 并行性回归 / 通稿）见 `RESOLUTION_2026-09-17.md`**。见 §11。
...
- `RESOLUTION_2026-09-17.md` —— **疑似解决里程碑**（根因/修复/A-B + **未结案条件**：单元压测/并行性回归/通稿）。
