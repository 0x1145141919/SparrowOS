...
> 约束对象：AI（Raven / OpenCode 等），非人类开发者。

### 11.1 拒绝 token 压缩类 skill

本项目的维护/重构工作与学习性开发中，**一律拒绝 token 压缩类 skill**（caveman、ADHD 类降效/压缩等）。

理由：省 token 不利于人机思维上下文对齐。维护工作本质是对齐 AI 与设计方的思维——设计意图、历史决策、踩坑记录；压缩即丢弃对齐材料，重构产物易偏离设计意图且偏差难以察觉。本项目注重内部设计与维护者视角，不是只看结果（许愿式编程）。

### 11.2 AI 思维可见性

学习性开发中，代码与文档应能体现 AI 的推理过程与设计意图，便于设计方审查思路；不追求「最小改动、最短输出」。设计意图、决策依据、踩坑记录要落到文档/注释里，而不是只留在对话里。
