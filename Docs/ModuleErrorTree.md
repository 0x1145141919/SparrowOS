...
**SRC_LOC 机制绝不替代 KURD**。两者服务不同目标：

| 机制 | 承载 | 用途 | 出现时机 |
|------|------|------|----------|
| `SRC_LOC()`（`abi/src_loc.h`） | 位置戳：(路径 hash, 行号) → u64 | 迭代开发期的错误极速定位 | 构建新功能时应急 |
| KURD（模块错误树） | 语义：(module→location→event→result→reason) | 运行时决策凭据、跨模块根因追溯 | 功能稳定后统一编排 |

### 为什么需要 SRC_LOC

开发期一边构思代码一边定义错误树（KURD），既不人类友好、更不 AI 友好：

- 语义枚举需要提前编排，而功能设计尚未定型时编排必然返工
- 错误树是契约，频繁改动侵蚀其 ABI 稳定性
- 让开发者 / AI 把注意力集中在**代码设计**本身

因此：**构建新功能时，出错点先用 `SRC_LOC()` 应急盖戳**，不定义任何语义。
出错即打印魔法值，宿主编译侧脚本 `build_utils/src_loc_decode.py` 还原 path:line。

### 什么时候升格为 KURD

功能逻辑定型、冷却稳定后，**统一**按模块错误树纪律编排 KURD：

- 设计方编排 in_module_location / 事件 / 原因
- AI 按错误树生成 KURD 构造代码
- 移除对应的 `SRC_LOC()` 应急盖戳

KURD 的语义承载（module→location→event→result→reason）是**运行时决策的凭据**：
调用方据此决定继续/重试/停机，跨模块根因可追溯——这是位置戳永远替代不了的。

### 纪律红线

- `SRC_LOC()` 是**开发期临时戳**，不是长期 API 的组成部分
- 稳定路径不得以 `SRC_LOC()` 代替 KURD 返回
- 升格 KURD 是**统一编排动作**，不随业务代码零散进行（配合约束三）
