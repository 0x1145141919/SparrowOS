...
> 状态：**draft（待设计方确认）**
> 版本：**v2 —— 按 2026-09-10 论点重构**（v1 见 git `0f077df`）
> 作者：Raven（AI 草案）
> 日期：2026-09-10
...
**妄图用一个 `bsp_kout` 覆盖所有场景。** 运行期（runtime）到来之后，不同场景的输出需求**已经分化**（kshell 只要屏幕、runtime 要异步日志、panic 要同步不可睡眠），却仍被挤进同一个全局对象——结果是 **backend 膨胀** + **kshell 难以改造**。其余问题都是在此之上的两个工程抉择。

> 类比 STL streams：`ifstream`/`ofstream`/`cout`/`cerr` 各司其职——不同输出需求 = 不同 stream。我们却把一个 stream 当成万能流。
...
- **A 面（结构）**：一个 `bsp_kout` 要同时服务早期、runtime、kshell、panic，被迫把所有场景的行为塞进一个 backend 结构 → **backend 膨胀**；且 kshell 只能借道日志管线 → **退格等 UI 需求难以实现**。
- **B 面（状态）**：`kout` 是 **BSS 全局对象**（多线程不友好），且内部携带**长生命周期进制子状态**，其编排范式 **AI 不友好**（谁改了状态、何时复位，靠人脑记忆）。
...
> 这一箭解决 **backend 膨胀 + kshell 难以改造**，是根因的直接解。
...
- [ ] **D0** 承认根因 ＝「单一 `bsp_kout` 的野心」，三箭中**箭一为结构性修复**、箭二/箭三为工程抉择。
...
*v2 由 AI 按设计方 2026-09-10 论点重构；推断（⚠️）与决策点（D*）需设计方确认后升为 spec。*
