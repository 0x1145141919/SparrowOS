...
// 5. 失败 → 回滚释放所有资源
// ============================================================
KURD_t NVMe_Controller::create_io_cq(uint16_t qid, uint16_t qsize,
                                      bool ien, KURD_t& kurd)
{
    // ---- 1. 分配 ring buffer ----
    ...
    if (error_kurd(kurd) || !cq_ring_va) return kurd;
...
// 4. 失败 → 回滚释放所有资源
// ============================================================
KURD_t NVMe_Controller::create_io_sq(uint16_t qid, uint16_t qsize,
                                      ...
                                      KURD_t& kurd)
{
    ...
    if (error_kurd(kurd) || !sq_ring_va) return kurd;
...
// ============================================================
KURD_t NVMe_Controller::io_queue_init(uint16_t iosq_count,
                                       uint16_t iocq_count,
                                       KURD_t& kurd)
{
    ...
    kurd = set_features_num_queues(iosq_count, iocq_count, kurd);
    if (error_kurd(kurd)) {
    ...
    for (uint16_t qid = 1; qid <= iocq_count; qid++) {
        kurd = create_io_cq(qid, IO_CQ_ENTRY_COUNT, true, kurd);
        if (error_kurd(kurd)) {
    ...
    for (uint16_t sqid = 1; sqid <= iosq_count; sqid++) {
        ...
        kurd = create_io_sq(sqid, IO_SQ_ENTRY_COUNT, cqid,
                             NVMe::io_queue::SQ_PRIO_HIGH, kurd);
        if (error_kurd(kurd)) {
...
// ============================================================
KURD_t NVMe_Controller::io_queue_free(KURD_t& kurd)
{
    // ---- 1. 删除所有 I/O SQ ----
