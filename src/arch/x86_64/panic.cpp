...
// ── 工具宏：统一提取 IDT 框架的 CS ──
// 在 x64_standard_context_v2 中, IDT 事件递送的 iret 帧通过 core_ctx.idtctx.iret 访问
void panic_frame(x64_standard_context_v2* frame, panic_context::x64_context* panic_frame)
{
    ...
    panic_frame->rip = IDT_RIP(frame);
    panic_frame->cs = static_cast<uint16_t>(IDT_CS(frame));
    panic_frame->rflags = IDT_RFLAGS(frame);
    panic_frame->rsp = IDT_RSP(frame);
    panic_frame->ss = static_cast<uint16_t>(IDT_SS(frame));
