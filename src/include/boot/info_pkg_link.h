...
// bcb_table 存"包基址相对偏移"（info_offset_t 语义），properties 内每条
// asset_entry_t 的 name/data 同样如此。链接 = 按包基址自加还原为可访问线性地址。
//
// 原地链接（单参数）：
...
//   - bcb_desc_v2_t::bitmap_region_base_pa 已是 fpa_bitmaps 线性地址
//     （info_fill.cpp 序列化时重定位），不参与重链
